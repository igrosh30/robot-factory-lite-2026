...
void fsm_round2::next_state_rules()
{
    ...
    if(state == M_SYS_START)
    {
        ...
        if(total_greens > 0) robot.send_command_param(NodeId::SLAVE_00,CMD_ID::INFO_GREEN_BOX, this->total_greens);
        else if(total_blues > 0) robot.send_command_param(NodeId::SLAVE_00, CMD_ID::INFO_BLUE_BOX, this->total_blues);//should be 4 blues! 
        if(total_greens > 0 )
        {
...
void fsm_round2::state_actions_rules()
{
    ...
    else if(state == M_SYS_LEAVE_START)
    {
        ...
        {
            robot.send_command_param(NodeId::SLAVE_00,INFO_BLUE_PICK_SLOT, S_blue_PICK[0]);
        }
